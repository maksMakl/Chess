    ...
    public class Pawn : Piece
    {
        ...
        public override void UnmarkSquares(int i, int j, ChessboardSquare[,] squares)
        {
            ...
            if (0 <= i + dir && i + dir <= 7)
            {
                if (squares[i + dir, j].Piece == null)
                {
                    squares[i + dir, j].SetDefault();
                }

                if (0 <= j + 1 && j + 1 <= 7 && squares[i + dir, j + 1].Piece != null && squares[i + dir, j + 1].Piece.Color != Color)
                {
                    squares[i + dir, j + 1].UnmarkCapture();
                }
                if (0 <= j - 1 && j - 1 <= 7 && squares[i + dir, j - 1].Piece != null && squares[i + dir, j - 1].Piece.Color != Color)
                {
                    squares[i + dir, j - 1].UnmarkCapture();
